function THDLoginPage_validate() {
    ...
    if (THDLoginPage_checkStoreNumber()) {
...
function THDLoginPage_checkStoreNumber() {
    storeNum = parent.document.THDLoginPage.j_storenumber.value;
    userBelongsTo = parent.document.THDLoginPage.j_userbelongsto.value;
    if (userBelongsTo === 'Store') {
        if (storeNum !== null & storeNum !== "") {
            if (storeNum.length === 4) {
                return true;
            } else {
                alert('Please enter a 4-digit Store Number.');
                return false;
            }
        } else {
...
function logoutUser() {
    ...
    for (var i = 0; i < cookies.length; i++) {
        ...
        if (name !== null && $.trim(name) === 'THDSSO') {
            document.cookie = name + "=; expires=Thu, 01 Jan 1970 00:00:00 GMT; path=/;Max-Age=0";
        } else if (name !== null && $.trim(name) === 'JSESSIONID') {
            document.cookie = name + "=; expires=Thu, 01 Jan 1970 00:00:00 GMT; path=/EnterpriseServiceRepository;Max-Age=0";
        }
    }
    $("#welcomeSpan").empty();
    $("#welcomeSpan").hide();
